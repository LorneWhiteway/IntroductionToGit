...
\begin{frame}{The index}
  \begin{block}{}
    \begin{itemize}
      \item{A git repository contains an \textit{index}, which is a list of the files in the working directory that git knows about (i.e. that git is `tracking'.}
      \item{The \textit{index} contains other information that makes it fast to answer the question `has this file changed since a version of it was last put into the repository?'}
    \end{itemize}
  \end{block}
  \begin{block}{}
    \begin{center}
...
\begin{frame}{Repository $\leftrightarrow$ working directory (1)}
  \begin{block}{Things you can do:}
    \begin{itemize}
      ...
      \item{Tell the repository about a new file in the working directory that you want the repository to pay attention to: \code{git add <filename>}. This does not do a commit!}
      \item{Make the working directory be equal to the contents of a specific node: \code{git checkout <nodename>}. This fails if there are uncommited changes in the working directory.}
    \end{itemize}
  \end{block}
\end{frame}
